...
Sophus::SE3f Tracking::GrabImageRGBDM(const cv::Mat &imRGB,const cv::Mat &imD, const cv::Mat &imMask, const double &timestamp, string filename)
{
    mImRGB   = imRGB;
    mImGray  = imRGB;
    mImDepth = imD;
    ...
    */    
    ...
    TICKTRACK("Track"); 
    mCurrentFrame = Frame(mImGray,mImDepth,mImMask,timestamp,mpLineExtractorLeft,mpORBextractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera);
