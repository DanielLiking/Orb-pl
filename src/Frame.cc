...
/// < RGBD with mask
Frame::Frame(const cv::Mat &imGray, const cv::Mat &imDepth, const cv::Mat &imMask, const double &timeStamp, 
             std::shared_ptr<LineExtractor>& lineExtractor, 
             ORBextractor* extractor,ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth, GeometricCamera* pCamera,Frame* pPrevF, const IMU::Calib &ImuCalib)
    :mpcpi(NULL),
     mpLineExtractorLeft(lineExtractor),mpLineExtractorRight(0),
     ...
     mbHasPose(false), mbHasVelocity(false)
{
    ...
    {        
        ...
        else
        {
            ...
            //过滤mask内部直线 TODO
            // FilterLinesByMask(imMask, mvKeyLines, mLineDescriptors);
...
// void Frame::FilterLinesByMask(const cv::Mat& imMask,
//                        std::vector<cv::line_descriptor_c::KeyLine>& mvKeyLines,
//                        cv::Mat& mLineDescriptors) {
//     std::vector<cv::line_descriptor_c::KeyLine> newKeyLines;
//     cv::Mat newLineDescriptors;

//     // 确保掩码和描述子矩阵不为空，并且掩码的尺寸正确
//     if (imMask.empty() || mLineDescriptors.empty()) {
//         std::cerr << "Mask or line descriptors matrix is empty." << std::endl;
//         return;
//     }

//     for (size_t i = 0; i < mvKeyLines.size(); ++i) {
//         bool isLineInMask = false;

//         // 检查直线的起点和终点是否在掩码区域内
//         cv::Point pt1(static_cast<int>(mvKeyLines[i].startPointX), static_cast<int>(mvKeyLines[i].startPointY));
//         cv::Point pt2(static_cast<int>(mvKeyLines[i].endPointX), static_cast<int>(mvKeyLines[i].endPointY));

//         // 确保坐标在掩码的范围内
//         if (pt1.x >= 0 && pt1.x < imMask.cols && pt1.y >= 0 && pt1.y < imMask.rows &&
//             (imMask.at<uchar>(pt1.y, pt1.x) == 1)) {
//             isLineInMask = true;
//         }
//         if (pt2.x >= 0 && pt2.x < imMask.cols && pt2.y >= 0 && pt2.y < imMask.rows &&
//             (imMask.at<uchar>(pt2.y, pt2.x) == 1)) {
//             isLineInMask = true;
//         }

//         // 如果直线不在掩码区域内，则保留它
//         if (!isLineInMask) {
//             newKeyLines.push_back(mvKeyLines[i]);
//             // 确保 i 在描述子矩阵的行数范围内
//             if (i < mLineDescriptors.rows) {
//                 newLineDescriptors.push_back(mLineDescriptors.row(i));
//             } else {
//                 std::cerr << "Index out of range when accessing line descriptors." << std::endl;
//                 break;
//             }
//         }
//     }
    ...
    // 更新直线和描述子
//     mvKeyLines = newKeyLines;
//     if (!newLineDescriptors.empty()) {
//         mLineDescriptors = newLineDescriptors;
//     } else {
//         mLineDescriptors = cv::Mat::zeros(mLineDescriptors.rows, mLineDescriptors.cols, mLineDescriptors.type());
//     }
// }
